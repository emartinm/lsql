...
class ViewsTest(TestCase):
    ...
    def test_not_logged(self):
        ...
        self.assertEqual(response.redirect_chain, [(collections_url, 302), (login_redirect_collections_url, 302)])

        # Collections redirects to login
        response = client.get(collections_url, follow=True)
        self.assertEqual(response.redirect_chain, [(login_redirect_collections_url, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_collection_url, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_problem, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_submit, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_submissions, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_submission, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_create, 302)])
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_results, 302)])
    ...
    def test_show_result_classification_date(self):
        ...
        response = client.get(
            classification_url,
            {"group": group_a.id, "start": start, "end": last_end_date},
            follow=True,
        )
        ...
        response = client.get(classification_url, {"group": group_a.id, "start": end, "end": start}, follow=True)
        self.assertIn(
        ...
        response = client.get(
            classification_url,
            {"group": group_a.id, "start": good_start_date, "end": end},
            follow=True,
        )
        self.assertEqual(response.status_code, HTTPStatus.OK)
        response = client.get(
            classification_url,
            {"group": group_a.id, "start": start, "end": wrong_end_date},
            follow=True,
        )
        self.assertIn(
        ...
        response = client.get(classification_url, {"group": group_a.id, "start": "eee", "end": end}, follow=True)
        self.assertIn("Introduzca una fecha válida", response.content.decode("utf-8"))

        response = client.get(classification_url, {"group": group_a.id, "start": start, "end": start}, follow=True)
    ...
    def test_first_day_of_course(self):
        ...
        self.assertEqual(
            datetime(2020, 9, 1).astimezone(),
            first_day_of_course(datetime(2020, 10, 2).astimezone()),
        )
        self.assertEqual(
            datetime(2020, 9, 1).astimezone(),
            first_day_of_course(datetime(2020, 9, 1)).astimezone(),
        )
        self.assertEqual(
            datetime(2019, 9, 1, 0, 0).astimezone(),
            first_day_of_course(datetime(2020, 3, 2)).astimezone(),
        )
        self.assertEqual(
            datetime(2020, 9, 1, 0, 0).astimezone(),
            first_day_of_course(datetime(2021, 7, 25)).astimezone(),
        )
        self.assertEqual(
            datetime(2021, 9, 1, 0, 0).astimezone(),
    ...
    def test_show_result_classification(self):
        ...
        response = client.get(classification_url, {"group": 999, "start": start, "end": end}, follow=True)
        self.assertEqual(response.status_code, HTTPStatus.NOT_FOUND)

        # I connect to a non-numeric group
        response = client.get(classification_url, {"group": "1A", "start": start, "end": end}, follow=True)
    ...
    def test_filter_expected_db(self):
        ...
        initial = {
            "ESTA SE BORRA": {
                "rows": [["11111X", "Real", "Concha", 70000]],
                "header": [
                    ["CIF", "<cx_Oracle.DbType DB_TYPE_CHAR>"],
            ...
            "ESTA SE MODIFICA": {
                "rows": [["111X", "222X", "004X"]],
                "header": [
                    ["CIF_LOCAL", "<cx_Oracle.DbType DB_TYPE_CHAR>"],
        ...
        expected = {
            "ESTA SE AGREGA": {
                "rows": [["11111X", "Gandia", "Guillermo Olague", 70000]],
                "header": [
                    ["CIF", "<cx_Oracle.DbType DB_TYPE_CHAR>"],
                    ["NOMBRE", "<cx_Oracle.DbType DB_TYPE_VARCHAR>"],
                    ["SEDE", "<cx_Oracle.DbType DB_TYPE_VARCHAR>"],
    ...
    def test_statistics(self):
        ...
        self.assertEqual(response.redirect_chain, [(login_redirect_stats_url, 302)])
        client.logout()
