...
class ModelsTest(TestCase):
    ...
    def test_problem_collection_stats(self):
        ...
        user2 = user_model.objects.create_user(username="usuario2", email="algodistinto@ucm.es", password="1234")  # nosec B106
        problem1.clean()
        problem1.save()
        problem2.clean()
        problem2.save()
    ...
    def test_judge_multiple_db(self):
        ...
        problem = SelectProblem(
            title_md="Test Multiple db Select",
            text_md="bla",
            create_sql=create,
            insert_sql=insert,
            collection=collection,
        ...
        self.assertEqual(
            problem.judge("SELECT Sede, Nombre FROM Club WHERE Nombre ='Madrid';", oracle)[0],
            VerdictCode.WA,
        )
    ...
    def test_solved_n_position(self):
        ...
        staff_user = get_user_model().objects.create_user("teacher", password="1234", is_staff=True, is_active=True)  # nosec B106
        inactive_user = get_user_model().objects.create_user(
            "inactive", password="1234", is_staff=False, is_active=False
        )  # nosec B106
        user = get_user_model().objects.create_user("normal", password="1234", is_staff=False, is_active=True)  # nosec B106
    ...
    def test_ObtainedAchievement(self):
        """Test to check that ObtainedAchievement is converted to string"""
        user = get_user_model().objects.create_user("student", password="1234")  # nosec B106
        achievement_definition = AchievementDefinition(name={"es": "nombre"}, description={"es": "descripcion"})
        achievement_definition.save()
        obtained_date = timezone.now()
        oa = ObtainedAchievement(user=user, achievement_definition=achievement_definition, obtained_date=obtained_date)

        expected = f"ObtainedAchievement {achievement_definition} ({obtained_date}) user={user}"
        self.assertEqual(str(oa), expected)
