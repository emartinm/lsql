...
class ShellTest(TestCase):
    ...
    CSV_BAD_FORMAT = [
        "csv_empty_document.csv",
        "csv_empty_first.csv",
        "csv_empty_name.csv",
        "csv_empty_email.csv",
        "csv_empty_last.csv",
        "csv_non_ucm_email.csv",
    ]
    ...
    def test_create_users_from_list_bad(self):
        ...
        with self.assertRaises(AssertionError):
            create_users_from_list(
                dict_list=[{"CORREO": "", "DOCUMENTO": "58962147S", "NOMBRE COMPLETO": "Calabaza, Manuel"}],
                group="test",
                dry=True,
            )
        ...
        with self.assertRaises(AssertionError):
            create_users_from_list(
                dict_list=[{"CORREO": "pep@ucm.es", "DOCUMENTO": "", "NOMBRE COMPLETO": "Calabaza, Manuel"}],
                group="test",
                dry=True,
            )
        ...
        with self.assertRaises(AssertionError):
            create_users_from_list(
                dict_list=[{"CORREO": "pep@ucm.es", "DOCUMENTO": "58962147S", "NOMBRE COMPLETO": ",Manuel"}],
                group="test",
                dry=True,
            )
    ...
    def test_adapt_db_expected_list(self):
        ...
        problems = [
            ...
            SelectProblem(
                ...
                author=None,
                expected_result=[{}],
            ),  # already has the right representation
        ]
...
class KeywordTest(TestCase):
    ...
    def test_is_projection(self):
        ...
        test_false = [
            "SELECT * FROM FROM",  # Syntactically invalid
            "SELECT * FROM Club",
            "SELECT * FROM Club JOIN Person WHERE 1 = 3 ORDER BY Club.ID",
            "SELECT * FROM Club WHERE ID NOT IN (SELECT * FROM Person)",
        ]
        test_true = [
            "SELECT ID FROM Club",
            "SELECT Name, ID FROM Club",
            "SELECT Club.Name, Person.ID FROM Club LEFT JOIN Person",
    ...
    def test_is_where(self):
        ...
        test_false = [
            "SELECT * FROM FROM",  # Syntactically invalid
            "SELECT * FROM Club",
            "SELECT * FROM Club JOIN Person ON ID1 = ID2 ORDER BY Club.ID",
    ...
    def test_is_inner_join(self):
        ...
        test_true = [
            "SELECT * FROM Club JOIN Person ON ID1 = ID2",
    ...
    def test_is_outer_join(self):
        ...
        test_true = [
            "SELECT * FROM Club LEFT JOIN Person ON ID1 = ID2",
    ...
    def test_is_set(self):
        ...
        test_true = [
            "(SELECT A, B, COUNT(*) FROM Club) UNION (SELECT * FROM Me)",
    ...
    def test_is_having(self):
        ...
        test_true = [
            "SELECT ID FROM Club GROUP BY ID HAVING COUNT(*) > 3",
    ...
    def test_is_nested(self):
        ...
        test_true = [
            "SELECT ID, (SELECT MAX(ID) FROM Club) FROM Club",
    ...
    def test_is_exists(self):
        ...
        test_true = [
            "SELECT A, NVL2(B, 3) FROM Club WHERE EXISTS (SELECT * FROM Club)",
    ...
    def test_is_like(self):
        ...
        test_true = [
            "SELECT A, NVL2(B, 3) FROM Club WHERE Name LIKE 'Pe%'",
    ...
    def test_keywords(self):
        ...
        self.assertEqual(
            keywords("(SELECT A, B, COUNT(*) FROM Club) UNION ALL (SELECT * FROM Me)"),
        ...
        self.assertEqual(
            keywords("""SELECT C.CIF, C.Nombre, NVL(AVG(Cantidad),0) AS PromedioFinanciaciones,
        ...
        self.assertEqual(
            keywords("""select a.NIF as NIF, p.Nombre as NOMBRE,
        ...
        self.assertEqual(
            keywords("""SELECT P.NIF, P.Nombre
