...
from judge.des_driver import (
    DesExecutor,
    filter_unrecognized_start_of_input,
    parse_tapi_cmd,
    parse_tapi_commands,
)
from judge.exceptions import DESException
from judge.models import DMLProblem, SelectProblem
...
CREATE VIEW v4 AS (
  SELECT DISTINCT nick, edad, ciudad 
  FROM partidas NATURAL JOIN jugadores, 
       (SELECT MAX(edad) as maximo 
        FROM (SELECT nick, edad, ciudad FROM partidas NATURAL JOIN 
              jugadores WHERE juego = 'Tetris')
       ) 
  WHERE edad = maximo and juego = 'Tetris')

CREATE VIEW v6 AS (
  SELECT nick 
  FROM (SELECT * FROM partidas) 
        DIVISION 
        (SELECT juego FROM partidas WHERE juego = 'Tetris'));"""
...
"""
    ...
    def test_validate_correct_problem(self):
        ...
        problem = SelectProblem(create_sql="CREATE TABLE t(age int)", insert_sql="", solution="SELECT * FROM t")
    ...
    def test_validate_wrong_problem(self):
        ...
        problem = SelectProblem(create_sql="CREATE TABLE t(age int);", insert_sql="", solution="SELECT * FROM table;")
...
"""
    ...
    def test_safe_des(self):
        ...
        safe = [
            "CREATE TABLE t(name VARCHAR(20) PRIMARY KEY, age INT); INSERT INTO t VALUES ('pepe', 3); " 
              "INSERT INTO t VALUES ('ana', 13);",
            "CREATE TABLE t(name VARCHAR(20) PRIMARY KEY, age INT); INSERT INTO t VALUES ('pepe', 3); "
              "SELECT * FROM Club",
            "SELECT e.first_name, e.last_name FROM employees e WHERE e.salary > " 
              "(SELECT AVG(inner_e.salary) FROM employees inner_e WHERE inner_e.department_id = e.department_id) "
              "AND e.department_id IN (SELECT d.department_id FROM departments d WHERE d.location = 'New York' "
              "AND (SELECT COUNT(*) FROM employees e2 WHERE e2.department_id = d.department_id) > 5);",
            """-- Create Tables
...
INSERT INTO courses (course_name, credits, department) VALUES ('Database Systems', 4, 'CS'), "
  "('Algorithms', 4, 'CS'), ('Calculus', 3, 'Math');
