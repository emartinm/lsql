...
def create_an_achievement_of_each(coll):
    ...
    ach_solved = NumSolvedAchievementDefinition(
        name={"es": "Resolvista"}, description={"es": "Resuelve 1 problema"}, num_problems=1
    )
...
class RankingTest(TestCase):
    ...
    def test_get_podium_achievement(self):
        ...
        podium_achievement = PodiumAchievementDefinition(
            name={"es": "Top 1"}, description={"es": "Se el primero"}, num_problems=1, position=1
        )
    ...
    def test_get_solved_achievement(self):
        ...
        solved_achievement_1 = NumSolvedAchievementDefinition(
            name={"es": "Solo 1"}, description={"es": "Acierta 1"}, num_problems=1
        )
        solved_achievement_2 = NumSolvedAchievementDefinition(
            name={"es": "Solo 2"}, description={"es": "Acierta 2"}, num_problems=2
        )
    ...
    def test_get_collection_achievement(self):
        ...
        coll_achievement = NumSolvedCollectionAchievementDefinition(
            name={"es": "Solo 1"}, description={"es": "Acierta 1"}, num_problems=1, collection=coll
        )
    ...
    def test_signal_update_achievement(self):
        ...
        ach_solved = NumSolvedAchievementDefinition(
            name={"es": "Resolvista"}, description={"es": "Resuelve 50 problemas"}, num_problems=50
        )
        ach_solved.save()
        ...
        ach_type = NumSolvedTypeAchievementDefinition(
            ...
            num_problems=1,
            problem_type=ProblemType.PROC.name,
        )
        ach_type.save()
        # Create NumSubmissionsProblemsAchievementDefinition
        ach_submi_pro = NumSubmissionsProblemsAchievementDefinition(
            name={"es": "Muchos envios"},
    ...
    def test_obtained_achievements_date(self):  # pylint: disable=too-many-statements
        ...
        ach_solved = NumSolvedAchievementDefinition(
            name={"es": "Resolvista"}, description={"es": "Resuelve 2 problemas"}, num_problems=2
        )
        ach_solved.save()
        ...
        ach_coll = NumSolvedCollectionAchievementDefinition(
            name={"es": "Coleccionista"},
            description={
                "es": "Resuelve 2 \
                                                            problemas de esta coleccion"
    ...
    def test_download_ranking(self):
        ...
        self.assertEqual(response.get("Content-Type"), "application/vnd.oasis.opendocument.spreadsheet")
        ...
        response = client.get(url, {"group": group_a.id, "start": end, "end": start}, follow=True)
        self.assertIn(
        ...
        response = client.get(
            url,
            {"group": group_a.id, "start": start, "end": datetime(4000, 7, 7).strftime("%Y-%m-%d")},
            follow=True,
        )
        self.assertIn(
            "La fecha final no puede ser mayor que la fecha de hoy",
            response.content.decode("utf-8"),
        )
        response = client.get(url, {"group": group_a.id, "start": start, "end": ""}, follow=True)
        self.assertIn("Este campo es obligatorio", response.content.decode("utf-8"))
        response = client.get(url, {"group": group_a.id, "start": "eee", "end": end}, follow=True)
        self.assertIn("Introduzca una fecha válida", response.content.decode("utf-8"))
        response = client.get(url, {"group": group_a.id, "end": end}, follow=True)
        self.assertIn("Este campo es obligatorio", response.content.decode("utf-8"))
        response = client.get(url, {"group": "1A", "start": start, "end": end}, follow=True)
