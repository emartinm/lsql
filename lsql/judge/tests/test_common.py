...
def create_user(passwd, username="usuario", email="email@ucm.es"):
    ...
    user = django.contrib.auth.get_user_model().objects.create_user(username=username, email=email, password=passwd)
    return user


def create_superuser(passwd, username="staff"):
    """Creates and stores a super user"""
    user = django.contrib.auth.get_user_model().objects.create_superuser(username=username, password=passwd)
    return user
