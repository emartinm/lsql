...
from .statistics import participation_per_group, submission_count, submissions_by_day
from .types import OracleStatusCode, ProblemType, VerdictCode
...
def show_result(request, collection_id):
    ...
    start = (
        first_day_of_course(datetime.today())
        if start is None
        else datetime(year=start.year, month=start.month, day=start.day)
    )
...
def show_collections(request):
    ...
    filter_groups = {
        group for teacher in get_user_model().objects.filter(is_staff=True) for group in teacher.groups.all()
    }
    return render(
        request,
        "collections.html",
        {"collections": cols, "filter_groups": filter_groups, "group_name": group_name},
    )
...
def show_problem(request, problem_id):
    ...
    problem.show_added, problem.show_modified, problem.show_removed = filter_expected_db(
        problem.expected_result[0], problem.initial_db[0]
    )
...
def show_submissions(request):
    ...
    if form.is_valid():
        ...
        start = (
            datetime(start_form.year, start_form.month, start_form.day)
            if start_form is not None
            else datetime(1950, 1, 1)
        )  # Very old date as default start
        end_form = form.cleaned_data.get("end")
        end = datetime(end_form.year, end_form.month, end_form.day) if end_form is not None else datetime.today()
        if problem is not None:
            subs = Submission.objects.filter(
                user=user,
                problem=problem.id,
                creation_date__range=[start.astimezone(), end.astimezone() + timedelta(days=1)],
            ).order_by("-pk")
        else:
            subs = Submission.objects.filter(
                user=user,
...
def show_achievements(request, user_id):
    ...
    achievements_definitions_unlocked = ObtainedAchievement.objects.filter(user=user).values_list(
        "achievement_definition", flat=True
    )
...
def submit(request, problem_id):
    ...
    * code: (string, required): code to be assessed
    ...
    data = {
        "verdict": VerdictCode.IE,
        "title": VerdictCode.IE.label,
        "message": VerdictCode.IE.message(),
        "feedback": "",
        "des": "",
    }
    code = ""
    ...
    submission = Submission(
        code=code[:5000],
        verdict_code=data["verdict"],
        verdict_message=data["message"],
        user=request.user,
