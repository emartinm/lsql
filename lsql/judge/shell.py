...
def create_users_from_list(dict_list, group: Group, dry: bool = True):
    ...
    for user_dict in dict_list:
        ...
        print(
            f'* Saved User("username:{username}", email:"{email}", passwd:"{password}", '
...
def rejudge(
    ...
    try:
        ...
        user = (
            rejudge_users[0]
            if len(rejudge_users) == 1
            else get_user_model().objects.create_user(username=username, password=passwd)
        )
        client = Client()
        ...
        with open(filename, "w", encoding="utf8") as report:
            for sub in subs:
                ...
                response = (
                    client.post(submit_url, {"code": sub.code}, HTTP_HOST=http_host, follow=True)
                    if not tests
...
def apply_sql_checker(
    sql: str, checker: Callable[[sqlglot.expressions.Expression], bool], dialect: str = "oracle"
) -> bool:
...
def is_aggregation(statement: sqlglot.expressions.Expression) -> bool:
    ...
    return (
        statement.find(sqlglot.expressions.Count) is not None
        or statement.find(sqlglot.expressions.Min) is not None
        or statement.find(sqlglot.expressions.Max) is not None
        or statement.find(sqlglot.expressions.Sum) is not None
        or statement.find(sqlglot.expressions.Avg) is not None
    )
...
def is_set(statement: sqlglot.expressions.Expression) -> bool:
    ...
    """
    return (
        statement.find(sqlglot.expressions.Union) is not None
        or statement.find(sqlglot.expressions.Intersect) is not None
        or statement.find(sqlglot.expressions.Except) is not None
    )
...
def is_null(statement: sqlglot.expressions.Expression) -> bool:
    ...
    """
    return (
        statement.find(sqlglot.expressions.Coalesce) is not None
        or statement.find(sqlglot.expressions.Null) is not None
        or statement.find(sqlglot.expressions.Nvl2) is not None
    )
