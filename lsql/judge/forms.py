...
class FunctionProblemAdminForm(forms.ModelForm):
    ...
    calls = forms.CharField(
        label="List of function calls to test (one per line)",
...
class ProcProblemAdminForm(forms.ModelForm):
    ...
    proc_call = forms.CharField(
        label="Procedure call to test (only one)",
        widget=forms.Textarea(attrs={"rows": 10, "cols": 80}),
        required=False,
    )
...
class TriggerProblemAdminForm(forms.ModelForm):
    ...
    tests = forms.CharField(
        label='SQL statements to test the trigger (separated by ";" as usual)',
        widget=forms.Textarea(attrs={"rows": 10, "cols": 80}),
        required=False,
    )
