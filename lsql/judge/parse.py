    ...
    with ZipFile(file) as zfile:
        ...
        if problem_type is not None and json_problem_type != problem_type:
            raise ZipFileParsingException(
                f'Invalid "type" field in {__JSON_NAME}: expected {problem_type} but {json_problem_type} obtained'
            )
    return problem_json
    ...
    try:
        ...
        with ZipFile(file) as zfile:
            if not __DML_PROBLEM_FILES <= set(zfile.namelist()):
                raise ZipFileParsingException(f"ZIP file must contain the following files: {__DML_PROBLEM_FILES}")
            ...
            problem.position = int(problem_json["position"])
            if (
                not problem.title_md
                or problem.min_stmt <= 0
                or problem.max_stmt <= 0
                or problem.min_stmt > problem.max_stmt
            ):
                raise ZipFileParsingException('Invalid value in JSON file: "title", "min_stmt" or "max_stmt"')
    ...
    try:
        ...
        with ZipFile(file) as zfile:
            if not __FUNCTION_PROBLEM_FILES <= set(zfile.namelist()):
                raise ZipFileParsingException(f"ZIP file must contain the following files: {__FUNCTION_PROBLEM_FILES}")
    ...
    try:
        ...
        with ZipFile(file) as zfile:
            if not __PROC_PROBLEM_FILES <= set(zfile.namelist()):
                raise ZipFileParsingException(f"ZIP file must contain the following files: {__PROC_PROBLEM_FILES}")
    ...
    try:
        ...
        with ZipFile(file) as zfile:
            if not __TRIGGER_PROBLEM_FILES <= set(zfile.namelist()):
                raise ZipFileParsingException(f"ZIP file must contain the following files: {__TRIGGER_PROBLEM_FILES}")
