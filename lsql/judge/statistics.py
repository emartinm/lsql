...
def submissions_by_day(start=None, end=None, verdict_code=None):
    ...
    days = list(
        map(
            lambda d: int(d.replace(hour=0, minute=0, second=0, microsecond=0).timestamp()) * 1000,
...
def submission_count():
    ...
    active_students = get_user_model().objects.filter(is_staff=False, is_active=True)
    counter = (
        Submission.objects.filter(user__in=active_students).values("verdict_code").annotate(count=Count("verdict_code"))
    )
...
def participation_per_group():
    ...
    for group in Group.objects.all():
        ...
        acc_count = (
            Submission.objects.filter(verdict_code=VerdictCode.AC, user__in=users)
            .order_by("user")
            .distinct("user")
            .count()
        )
        ...
        participating[group.name] = {
            ...
            "quantiles": " - ".join(
                map(
                    str,
                    [min(list_num_subs)] + quantiles(list_num_subs, n=4, method="inclusive") + [max(list_num_subs)],
                )
            )
            if len(list_num_subs) >= 5
            else "N/A",
        }
